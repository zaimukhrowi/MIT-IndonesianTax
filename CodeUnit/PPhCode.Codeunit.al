codeunit 60006 PPhCode
{
                                    ...
                                    if not WHTTrans.FindSet() then begin
                                        ...
                                        if (GeneralLedgerSetup."LCY Code" <> GLEntry."Source Currency Code") and (GLEntry."Source Currency Code" <> '') then begin
                                            ...
                                            if GLEntry."Document Type" = GLEntry."Document Type"::"Credit Memo" then
                                                WHTTrans."WHT Amount (Tax Rate)" := GLEntry.WHTAmount * -1 * CurrencyExchangeRate."Kre Tax Rate"
                                            else
                                                WHTTrans."WHT Amount (Tax Rate)" := GLEntry.WHTAmount * CurrencyExchangeRate."Kre Tax Rate";
                                        end else
                                            if GLEntry."Document Type" = GLEntry."Document Type"::"Credit Memo" then
                                                WHTTrans."WHT Amount (Tax Rate)" := GLEntry.WHTAmount * -1
                                            else
                                                            ...
                                                            begin
                                                                ...
                                                                if VendorLedger.FindSet() then begin
                                                                    ...
                                                                    if VendorLedger."Applies-to Doc. No." = '' then begin
                                                                        ...
                                                                        WHTTrans."DPP Amount (Tax Rate)" := 0;
                                                                        WHTTrans."VAT Amount" := 0;
                                                                        WHTTrans."VAT Amount (Tax Rate)" := 0;
                                                                    end else begin
                                                            ...
                                                            begin
                                                                ...
                                                                if PurchInvLine.FindSet() then begin
                                                                    ...
                                                                    if (GeneralLedgerSetup."LCY Code" <> PurchInvHeader."Currency Code") and (PurchInvHeader."Currency Code" <> '') then begin
                                                                        ...
                                                                        CurrencyExchangeRate.FindLast();
                                                                        WHTTrans."Amount (Tax Rate)" := System.Abs(PurchInvLine."Amount Including VAT") * CurrencyExchangeRate."Kre Tax Rate";
                                                                        WHTTrans."DPP Amount (Tax Rate)" := System.Abs(PurchInvLine."Amount") * CurrencyExchangeRate."Kre Tax Rate";
                                                                    end else begin
                                    ...
                                    if GLEntry."Source Code" = 'SALES' then begin
                                                                ...
                                                                begin
                                                                    ...
                                                                    if SalesInvLine.FindSet() then begin
                                                                        ...
                                                                        if (GeneralLedgerSetup."LCY Code" <> SalesInvHeader."Currency Code") and (SalesInvHeader."Currency Code" <> '') then begin
                                                                            ...
                                                                            CurrencyExchangeRate.FindLast();
                                                                            WHTTrans."Amount (Tax Rate)" := System.Abs(SalesInvLine."Amount Including VAT") * CurrencyExchangeRate."Kre Tax Rate";
                                                                            WHTTrans."DPP Amount (Tax Rate)" := System.Abs(SalesInvLine."Amount") * CurrencyExchangeRate."Kre Tax Rate";
                                                                        end else begin
                                        ...
                                        if GLEntry."Source Code" = 'CASHRECJNL' then begin
                                            ...
                                            if not WHTTrans.FindSet() then begin
                                                ...
                                                if SalesInvHeader.Get(GLEntry."WHT Source Document No.") then begin
                                                    WHTTrans."Pre-Assigned No." := SalesInvHeader."Pre-Assigned No.";
                                                    WHTTrans."Invoice Date" := SalesInvHeader."Posting Date";
                                                end;
                                                WHTTrans."Entry No" := GLEntry."Entry No.";
                                                WHTTrans."External Doc No" := GLEntry."External Document No.";
                                                WHTTrans."Global Dimension 1" := GLEntry."Global Dimension 1 Code";
    ...
    begin
        ...
        if GenJournalLine."Account Type" = GenJournalLine."Account Type"::Customer then begin
            ...
            repeat
                if Kre_MasterPPh.Get(SalesInvoiceLine.WHTProductPostingGroup) then begin
    ...
    begin
        ...
        PLine.Init();
        PLine.Validate("Document Type", DocType);
        PLine.Validate("Document No.", DocNo);
        PLine.Validate(Type, PLine.Type::"G/L Account");
        PLine.Validate("No.", GLNo);
        PLine.Description := StrSubstNo('%1 - %2', GLName, Description);
        PLine.Validate("Line No.", GetLastPNoLine(DocNo));
        // PLine.Validate("Line Amount", SumLineAmount * -1);
        PLine.Validate("Direct Unit Cost", SumLineAmount * -1);
        // PLine.Validate(Amount, SumLineAmount * -1;
        // PLine."Amount Including VAT" := SumLineAmount * -1;
        // PLine."VAT Base Amount" := SumLineAmount * -1;
        PLine.Validate("Dimension Set ID", DimSetID);
        ...
        PLine.Validate("Unit of Measure Code", Unitcode);
    ...
    begin
        ...
        PLine.Validate("Direct Unit Cost", WHTGross * -1);
    ...
    begin
        ...
        GJLine.Init();
        GJLine.Validate("Journal Template Name", Journal_Template_Name);
        GJLine.Validate("Journal Batch Name", Journal_Batch_Name);
        GJLine.Validate("Source Code", Source_Code);
        GJLine.Validate("Posting Date", Posting_Date);
        GJLine.Validate("Document Type", GJLine."Document Type"::Payment);
        GJLine.Validate("Document No.", DocNo);
        GJLine.Description := StrSubstNo('%1 - %2', GLName, Description);
        GJLine.Validate("Line No.", GetLastGJNoLine(DocNo, Journal_Template_Name, Source_Code, Journal_Batch_Name));
        GJLine.Validate("Account Type", GJLine."Account Type"::"G/L Account");
        GJLine.Validate("Amount", Abs(SumLineAmount));
        GJLine.Validate("Amount (LCY)", Abs(SumLineAmount));
        // GJLine."Shortcut Dimension 1 Code", ShortcutDimCode[1];
        // GJLine."Shortcut Dimension 2 Code", ShortcutDimCode[2];
        GJLine.Validate("Account No.", GLNo);
        GJLine.Validate("Currency Code", Currency_Code);
        GJLine.Validate(Quantity, 1);
