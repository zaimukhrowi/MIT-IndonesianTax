page 60021 PPhMasukanList
{
    PageType = List;
    SourceTable = KreWHTTrans;
    SourceTableView = sorting(ID) order(ascending) where("Source Type" = filter(Vendor));
    ApplicationArea = All;
    UsageCategory = Lists;
    Caption = 'PPh Masukan List';
    InsertAllowed = false;
    ...
    layout
    {
        area(Content)
        {
            repeater(PPhMasukan)
            {
                ...
                field("Remaining Amount"; Rec."Remaining Amount Vendor")
                {
                    ApplicationArea = All;
                    Caption = 'Remaining Amount';
                    Editable = false;
                }
                field("Remaining Amount LCY"; Rec."Remaining Amount Vendor LCY")
                {
                    ApplicationArea = All;
    ...
    var
        DetailVend: Record "Detailed Vendor Ledg. Entry";
        wht: Record Kre_MasterPPh;
    BEGIN
        DetailVend.Reset();
        DetailVend.SetRange("Vendor Ledger Entry No.", Rec."Entry No Ledger Entry Vend");
        DetailVend.SetRange("Document Type", DetailVend."Document Type"::Payment);
        if DetailVend.FindSet() then
            PaymentDate := DetailVend."Posting Date"
        else
            PaymentDate := 0D;
